...
const PM06 = (() => {
  ...
  function createProcessor(site, year) {
    ...
    function finish() {
      ...
      return {
        detail, combo, eoList, mtrPack, orderRows, usoSlices, vmzSlices, capexRows, n,
        oemPairs: [...oemPairs],
        meta: { site, year, rows: rawRows, orders: allOrders.size }
      };
    }
  ...
  function mergeIntoCube(cube, capexRows, results) {
    ...
    results.forEach(r => {
      ...
      const rec = { file: name, site: r.meta.site, year: Number(r.meta.year), rows: r.meta.rows, orders: r.meta.orders };
