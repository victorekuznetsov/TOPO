#!/bin/bash
# Пересборка БДО (pm06_meta/bdo) из исходных выгрузок ветки rawdata.
# Файлы скачиваются по одному, обрабатываются и сразу удаляются —
# все выгрузки (≈700 МБ) на диске одновременно не нужны.
#   pm06_meta/bdo/build_all.sh <рабочая_папка> && python3 pm06_meta/bdo/build_bdo.py <рабочая_папка> pm06_meta/bdo
set -u
...
mkdir -p "$WORK"
one(){ name=$1; shift
  [ -f "$WORK/$name.json.gz" ] && { echo "$name: уже есть"; return; }
  ext=zip; [[ "$1" == *.xlsx ]] && ext=xlsx
  tmp="$WORK/_src.$ext"; urls=(); for p in "$@"; do urls+=("$BASE/$p"); done
  for t in 1 2 3; do
    curl -sSfL "${urls[@]}" > "$tmp" && python3 "$HERE/scan_pm06_bdo.py" "$tmp" "$name" "$WORK/$name.json.gz" && break
